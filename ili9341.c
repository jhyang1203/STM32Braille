...
static void ILI9341_WriteChar(uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color, uint16_t bgcolor) {
    ...
    for (i = 0; i < font.height; i++) {
        ...
        for (j = 0; j < font.width; j++) {
            uint8_t pixel_on = (b & (1 << (15 - j))) ? 1 : 0;

            for (dy = 0; dy < font.scale_y; dy++) {
                for (dx = 0; dx < font.scale_x; dx++) {
                    uint8_t data[] = {
...
void ILI9341_WriteString(uint16_t x, uint16_t y, const char* str, FontDef font, uint16_t color, uint16_t bgcolor) {
    ...
    while (*str) {
        ...
        if (x_start + font.width * font.scale_x >= ILI9341_WIDTH) {
            ...
            if (y_start + font.height * font.scale_y >= ILI9341_HEIGHT) {
                break;
            }
        ...
        ILI9341_WriteChar(x_start, y_start, *str, font, color, bgcolor);
        x_start += font.width * font.scale_x;  // 문자 간격도 확대 배수 적용
        str++;
    }
